/**
 * /api/refresh-static — Vercel serverless function to refresh static data files
 *
 * ADMIN only: Requires JWT token in Authorization header
 * Triggered by admin panel after product/brand changes.
 *
 * Usage: POST /api/refresh-static
 * Headers: Authorization: Bearer <admin-token>
 */
const path = require('path');
const fs = require('fs');

// Reuse the build logic
const buildScript = path.join(__dirname, '..', 'scripts', 'build-static.js');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Run the build (inline to avoid child_process issues on Vercel)
    const result = await runInlineBuild();
    ...
    console.error('[refresh-static] Error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
};

// ============= Inline Build (duplicated from scripts/build-static.js for Vercel compatibility) =============
...
async function runInlineBuild() {
  ...
  if (useMongo) {
    try {
      ...
      try {
        ...
        brands = brands.map(b => ({ id: b.id || b._id.toString(), name: b.name, slug: b.slug, image: b.image || '', icon: b.icon || '' }));
      } catch (e) { brands = SEED_BRANDS; }

      await client.close();
    } catch (e) { products = SEED_PRODUCTS; brands = SEED_BRANDS; }
  } else {
    products = SEED_PRODUCTS; brands = SEED_BRANDS;
  }
  ...
  fs.writeFileSync(path.join(STATIC_DIR, 'products.json'), JSON.stringify({
    lastBuilt: new Date().toISOString(), count: activeProducts.length, products: activeProducts,
  ...
  fs.writeFileSync(path.join(STATIC_DIR, 'brands.json'), JSON.stringify({
    lastBuilt: new Date().toISOString(), count: brands.length, brands
  }, null, 2));

  const elapsed = Date.now() - startTime;
  return { elapsed, products: activeProducts.length, featured: featuredProducts.length, specialOffers: specialOfferProducts.length, brands: brands.length };
}
